...
#volume correction force calculation with 32 bit data types, for comparing results with GPU calculations that use 32 bit data types. I need to calculate all the forces acting on each vertex (or gradient of the energy wrt the vertex position, which is the negative of the force vector acting on the vertex). I can write this to work for a single unit cell at first, but I need to think about how I will store the forces before I add them up, and assign them to the correct vertices.
@cython.boundscheck(False)
@cython.wraparound(False)
