...
# compare the numpy and cythonized volume correction force expressions for correctness and performance
import numpy as np
import timeit
...
#volume correction force calculation with numba. I need to calculate all the forces acting on each vertex (or gradient of the energy wrt the vertex position, which is the negative of the force vector acting on the vertex). I can write this to work for a single unit cell at first, but I need to think about how I will store the forces before I add them up, and assign them to the correct vertices.
# @numba.jit(nopython=True,parallel=True)
# def get_volume_correction_force_numba(elements, avg_vectors, kappa, l_e,N_vertices):
#     """calculate the volume correction force on each of the vertices of the unit cell"""
#     #maybe this should return the force on each vertex of the unit cell
#     correction_force = np.zeros((N_vertices,3),dtype=np.float64)
#     temp_result = {}
#     num_el = elements.shape[0]
#     #for i, el in enumerate(elements):
#     for i in numba.prange(num_el):
#         temp_result[i] = get_volume_correction_force_el_numba(avg_vectors[:,:,i], kappa, l_e)
#         #correction_force[el] += get_volume_correction_force_el_numba(avg_vectors[:,:,i], kappa, l_e)
#     for i in range(num_el):
#         correction_force[elements[i,:]] = temp_result[i]
#     return correction_force

# @numba.jit(numba.float64[:,:](numba.float64[:,:],numba.float64,numba.float64),nopython=True)
# def get_volume_correction_force_el_numba(avg_vec, kappa, l_e):
#     acrossb = np.cross(avg_vec[0,:],avg_vec[1,:])
#     bcrossc = np.cross(avg_vec[1,:],avg_vec[2,:])
#     ccrossa = np.cross(avg_vec[2,:],avg_vec[0,:])
#     adotbcrossc = np.dot(avg_vec[0,:],bcrossc)
...
#     correction_force_el[0,:] = prefactor * gradV1
#     correction_force_el[1,:] = prefactor * gradV2
#     correction_force_el[2,:] = prefactor * gradV3
#     correction_force_el[3,:] = prefactor * gradV4
#     correction_force_el[4,:] = prefactor * gradV5
#     correction_force_el[5,:] = prefactor * gradV6
#     correction_force_el[6,:] = prefactor * gradV7
#     correction_force_el[7,:] = prefactor * gradV8
#     return correction_force_el
...
def main():
    ...
    # assert (all(correction_force_npy == correction_force_cy))
    # if all(correction_force_npy == correction_force_cy):
    #     print('same correction force calculated between methods')
    ...
    # numba using equivalent of njit, run once to allow to compile, then time the result post compilation
    # correction_force_numba = get_volume_correction_force_numba(elements, avg_vectors, kappa, l_e,N)
    # start = time.perf_counter()
    # for i in range(N_runs_perf):
    #     correction_force_numba = get_volume_correction_force(elements, avg_vectors, kappa, l_e,N)

    # end = time.perf_counter()
    # delta_numba = end - start
    # print(delta_numba,delta_npy)
    # print('Numba is {}x faster'.format(delta_npy/delta_numba))
    ...
    correction_force_cy_nogil = np.empty((N,3),dtype=np.float64)
    start = time.perf_counter()
    for i in range(N_runs_perf):
